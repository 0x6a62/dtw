...
pub fn showMatrix(comptime T: type, writer: *std.Io.Writer, aMax: usize, bMax: usize, matrix: []const T) !void {
    for (0..aMax) |ai| {
        for (0..bMax) |bi| {
            try std.Io.Writer.print(writer, "{d:5.1} ", .{matrix[index(aMax, ai, bi)]});
        }
